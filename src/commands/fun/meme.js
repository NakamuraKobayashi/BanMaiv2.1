...
module.exports = {
  name: "meme",
  description: "nhận ngẫu nhiên một meme",
  category: "FUN",
...
async function getRandomEmbed(choice) {
  ...
  if (!response.success) {
    return new EmbedBuilder().setColor(EMBED_COLORS.ERROR).setDescription("Không tìm nạp được meme. Vui lòng thử lại!");
  }

  const json = response.data;
  if (!Array.isArray(json) || json.length === 0) {
    return new EmbedBuilder().setColor(EMBED_COLORS.ERROR).setDescription(`Không tìm thấy meme: ${choice}`);
  }
