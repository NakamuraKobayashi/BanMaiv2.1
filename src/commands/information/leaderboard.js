...
module.exports = {
  name: "leaderboard",
  description: "hiển thị bảng xếp hạng XP, lượt mời và điểm uy tín",
  ...
  slashCommand: {
    ...
    options: [
      {
        name: "type",
        description: "loại bảng xếp hạng",
        required: true,
  ...
  async messageRun(message, args, data) {
    ...
    switch (type) {
      ...
      case "rep":
        ...
        break;
      default:
        response = "Loại bảng xếp hạng không hợp lệ. Hãy chọn giữa `xp`, `invite` hoặc `rep`";
    }
...
async function getXpLeaderboard({ guild }, author, settings) {
  ...
  if (!settings.stats.enabled) return "Bảng xếp hạng bị vô hiệu hóa ở máy chủ này";

  const lb = await getXpLb(guild.id, 10);
  if (lb.length === 0) return "Không có người dùng nào trong bảng xếp hạng";
  ...
  const embed = new EmbedBuilder()
    .setAuthor({ name: "Bảng xếp hạng XP" })
    .setColor(EMBED_COLORS.BOT_EMBED)
    .setDescription(collector)
    .setFooter({ text: `Yêu cầu bởi ${author.tag}` });
...
async function getInviteLeaderboard({ guild }, author, settings) {
  ...
  if (!settings.invite.tracking) return "Invite Tracking bị vô hiệu hóa ở máy chủ này";

  const lb = await getInvitesLb(guild.id, 10);
  if (lb.length === 0) return "Không có ai trong bảng xếp hạng";
  ...
  const embed = new EmbedBuilder()
    .setAuthor({ name: "Bảng xếp hạng mời" })
    .setColor(EMBED_COLORS.BOT_EMBED)
    .setDescription(collector)
    .setFooter({ text: `Yêu cầu bởi ${author.tag}` });
...
async function getRepLeaderboard(author) {
  ...
  const embed = new EmbedBuilder()
    .setAuthor({ name: "Bảng xếp hạng danh tiếng" })
    .setColor(EMBED_COLORS.BOT_EMBED)
    .setDescription(collector)
    .setFooter({ text: `Yêu cầu bởi ${author.tag}` });
