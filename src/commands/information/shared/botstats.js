...
module.exports = (client) => {
  ...
  let desc = "";
  desc += `❒ Tổng số máy chủ: ${guilds}\n`;
  desc += `❒ Tổng số người dùng: ${users}\n`;
  desc += `❒ Tổng số kênh: ${channels}\n`;
  desc += `❒ Websocket Ping: ${client.ws.ping} ms\n`;
  desc += "\n";

  const embed = new EmbedBuilder()
    .setTitle("Thông tin về tớ")
  ...
  // Buttons
  let components = [];
  components.push(new ButtonBuilder().setLabel("Mời tớ").setURL(client.getInvite()).setStyle(ButtonStyle.Link));
