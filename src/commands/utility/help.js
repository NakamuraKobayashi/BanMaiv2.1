...
module.exports = {
  ...
  async messageRun(message, args, data) {
    ...
    await message.safeReply("Không tìm thấy kết quả tương ứng!");
  },
  ...
  async interactionRun(interaction) {
    ...
    // No matching command/category found
    await interaction.followUp("Không tìm thấy kết quả tương ứng!");
  },
};
...
async function getHelpMenu({ client, guild }) {
  ...
  for (const [k, v] of Object.entries(CommandCategory)) {
    ...
    options.push({
      label: v.name,
      value: k,
      description: `Xem danh mục ${v.name}`,
      emoji: v.emoji,
    });
  }
  ...
  const embed = new EmbedBuilder()
    ...
    .setDescription(
      "**Giới thiêu về tớ:**\n" +
        `Helu tớ là ${guild.members.me.displayName}!\n` +
        "Là một trợ thủ đắc lực dành cho bạn, phục vụ mọi nhu cầu của bạn\n\n" +
        `**Mời tớ:** [Nhấn vào đây](${client.getInvite()})\n` +
        `**Máy chủ hỗ trợ:** [Tham gia](${SUPPORT_SERVER})`
    );
...
function getSlashCategoryEmbeds(client, category) {
  ...
  arrSplitted.forEach((item, index) => {
    const embed = new EmbedBuilder()
      ...
      .setFooter({ text: `Trang ${index + 1} trên ${arrSplitted.length}` });
...
function getMsgCategoryEmbeds(client, category, prefix) {
  ...
  if (category === "IMAGE") {
    ...
    collector +=
      "\n\nBạn có thể sử dụng các lệnh hình ảnh này ở các định dạng sau\n" +
      `**${prefix}cmd:** Chọn ảnh đại diện của bạn làm hình ảnh\n` +
      `**${prefix}cmd <@member>:** Chọn ảnh đại diện của người được đề cập làm hình ảnh\n` +
      `**${prefix}cmd <url>:** Chọn ảnh từ đường link được cung cấp\n` +
      `**${prefix}cmd [attachment]:** Chọn hình ảnh đính kèm`;
  ...
  arrSplitted.forEach((item, index) => {
    const embed = new EmbedBuilder()
      ...
      .setDescription(item.join("\n"))
      .setFooter({
        text: `Trang ${index + 1} trên ${arrSplitted.length} | Gõ ${prefix}help <command-name> để xem chi tiết về lệnh`,
      });
    arrEmbeds.push(embed);
  });
