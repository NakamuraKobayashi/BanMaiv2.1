...
module.exports = {
  name: "unban",
  description: "hủy cấm một người chỉ định",
  category: "MODERATION",
...
async function getMatchingBans(guild, match) {
  ...
  if (matched.length === 0) return `Không tìm thấy người dùng ${match}`;
...
async function waitForBan(issuer, reason, sent) {
  ...
  collector.on("collect", async (response) => {
    ...
    if (typeof status === "boolean") return sent.edit({ content: `${user.username} đã được hủy cấm!`, components: [] });
    else return sent.edit({ content: `Thất bại khi hủy cấm ${user.username}`, components: [] });
  });

  // collect user and unban
  collector.on("end", async (collected) => {
    if (collected.size === 0) return sent.edit("Ầu! Không phản hồi trong thời gian chỉ định. Vui lòng thử lại sau.");
  });
}
