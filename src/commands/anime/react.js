...
module.exports = {
  name: "react",
  description: "Biểu tượng cảm xúc anime",
  enabled: true,
  category: "ANIME",
  cooldown: 5,
  command: {
  ...
  async messageRun(message, args) {
    ...
    if (!choices.includes(category)) {
      return message.safeReply(`Lựa chọn không hợp lệ: \`${category}\`.\nCác biểu cảm có sẵn: ${choices.join(", ")}`);
    }
...
const genReaction = async (category, user) => {
  try {
    ...
    return new EmbedBuilder()
      .setImage(imageUrl)
      .setColor("Random")
      .setFooter({ text: `Bởi ${user.tag}` });
  } catch (ex) {
    return new EmbedBuilder()
      .setColor(EMBED_COLORS.ERROR)
      .setDescription("Không tìm nạp được meme. Vui lòng thử lại!")
      .setFooter({ text: `Bởi ${user.tag}` });
  }
};
