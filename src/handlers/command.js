  ...
  handlePrefixCommand: async function (message, cmd, settings) {
    ...
    if (cmd.userPermissions && cmd.userPermissions?.length > 0) {
      if (!message.channel.permissionsFor(message.member).has(cmd.userPermissions)) {
        return message.safeReply(`Bạn cần có quyền ${parsePermissions(cmd.userPermissions)} để sử dụng lệnh này`);
      }
    }
    ...
    if (cmd.botPermissions && cmd.botPermissions.length > 0) {
      if (!message.channel.permissionsFor(message.guild.members.me).has(cmd.botPermissions)) {
        return message.safeReply(`Tớ cần quyền ${parsePermissions(cmd.botPermissions)} cho lệnh này`);
      }
    }
  ...
  handleSlashCommand: async function (interaction) {
    ...
    if (interaction.member && cmd.userPermissions?.length > 0) {
      if (!interaction.member.permissions.has(cmd.userPermissions)) {
        return interaction.reply({
          content: `Bạn cần quyền ${parsePermissions(cmd.userPermissions)} để sử dụng lệnh này`,
          ephemeral: true,
        });
      }
    }
    ...
    if (cmd.botPermissions && cmd.botPermissions.length > 0) {
      if (!interaction.guild.members.me.permissions.has(cmd.botPermissions)) {
        return interaction.reply({
          content: `Tớ cần quyền ${parsePermissions(cmd.botPermissions)} cho lệnh này`,
          ephemeral: true,
        });
      }
    }
    ...
    try {
      ...
      await interaction.followUp("Oops! Đã có lỗi xảy ra khi chạy lệnh này");
